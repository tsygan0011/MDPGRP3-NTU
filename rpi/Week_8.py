...
class RaspberryPi:
    ...
    def recv_android(self) -> None:
        ...
        while True:
            ...
            elif message['cat'] == "obstacles_fin":
                self.rpi_action_queue.put(PiAction(**message))
                self.logger.debug(f"Finish adding obstacles")
    ...
    def stitch(self):
        print("stitching...")
        file_path = r"/home/pi/MDP2/src/images/task1/results"
        save_dir = r"/home/pi/MDP2/src/images/task1/runs"
        count = 0
    ...
    def command_follower(self) -> None:
        ...
        while True:
            ...
            self.logger.debug("wait for unpause (command_follower_queue)")
            # Wait for unpause event to be true [Main Trigger]
            try:
                self.logger.debug("wait for retrylock")
                self.retrylock.acquire()
                self.retrylock.release()
                self.logger.debug("retrylock release")

            except:
                self.logger.debug("wait for unpause_cannot get retrylock")
                self.unpause.wait()
            ...
            if command.startswith(stm32_prefixes):
                ...
                self.logger.debug(f"glenn Sending to STM32: {command}")
    ...
    def rpi_action(self):
        ...
        while True:
            ...
            if action.cat == "obstacles":
                ...
                self.obs = action.value
#                self.request_algo(action.value)
            
            elif action.cat == "obstacles_fin":
                self.request_algo(self.obs)
    ...
    def snap_and_rec(self, obstacle_id_with_signal: str) -> None:
        ...
        # self.logger.debug("Requesting from image API")
        results = self.image_predict(obstacle_id)
#        response = requests.post(url, files={"file": (img_full_path, open(img_full_path, 'rb'))})
        ...
        # print("DEDBUGGGGGGGGGGGGGGGG2", results)
        ...
        if None:
            self.failed_obstacles.append(self.obstacles[str(results['obstacle_id'])])
            ...
            # print("DEDBUGGGGGGGGGGGGGGGG", results['obstacle_id'])
            self.success_obstacles.append(self.obstacles[str(results['obstacle_id'])])
            self.logger.info(f"self.success_obstacles: {self.success_obstacles}")
        self.android_queue.put(AndroidMessage("image-rec", results))

    def image_predict(self, obs_id=100):
        # save the image file to the uploads folder
        
        print("capturing image...")
        filename = f"{int(time.time())}_{obs_id}.jpg"
        captured_dir = r"/home/pi/MDP2/src/images/task1/captured/"
        ...
        image_id = objectdetection_yolov8.detect(img_full_path, 1)
